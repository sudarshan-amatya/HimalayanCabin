...
function buildPaymentResponse(amount: number, transactionUuid: string) {
  return {
...
export async function initiateBookingEsewaPayment(req: Request<{ id: string }>, res: Response) {
  try {
    if (!req.user) return res.status(401).json({ message: "Not authorized" });
    if (req.user.role !== "USER") return res.status(403).json({ message: "Only guest accounts can pay for bookings" });
...
export async function initiateGiftEsewaPayment(req: Request, res: Response) {
  try {
    if (!req.user) return res.status(401).json({ message: "Not authorized" });
    if (req.user.role !== "USER") return res.status(403).json({ message: "Only guest accounts can send gifts" });
    ...
    if (!normalizedRecipientEmail.includes("@")) return res.status(400).json({ message: "Please enter a valid recipient email" });
    ...
    if (recipient.id === req.user.id) return res.status(400).json({ message: "You cannot send a gift to yourself" });
    ...
    if (giftType === "VOUCHER") {
      ...
      if (!voucherAmount || voucherAmount < 1000) return res.status(400).json({ message: "Voucher amount should be at least Rs. 1,000" });
      ...
      if (!cabin || cabin.status !== "APPROVED" || !cabin.isActive) return res.status(404).json({ message: "Cabin not found, inactive, or not approved yet" });

      const checkIn = new Date(checkInDate);
      const checkOut = new Date(checkOutDate);
      if (Number.isNaN(checkIn.getTime()) || Number.isNaN(checkOut.getTime())) return res.status(400).json({ message: "Invalid check-in or check-out date" });

      if (startOfDay(checkIn) < startOfDay(new Date())) return res.status(400).json({ message: "Check-in date cannot be in the past" });
      if (checkOut <= checkIn) return res.status(400).json({ message: "Check-out date must be after check-in date" });
...
export async function esewaSuccess(req: Request, res: Response) {
  try {
    ...
    const cleanResponse = JSON.parse(JSON.stringify(decoded)) as Prisma.InputJsonValue;
    ...
    if (!payment) return redirect(res, "/payment/failure?reason=payment-not-found");
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (existingPayment.purpose === "BOOKING") {
        if (!existingPayment.bookingId) throw new Error("Booking missing for payment");
      ...
      const draft = existingPayment.giftDraft as GiftDraft | null;
      if (!draft) throw new Error("Gift draft missing for payment");
...
export async function esewaFailure(_req: Request, res: Response) {
