...
type CabinStatus = "PENDING" | "APPROVED" | "REJECTED";

const cabinStatuses: CabinStatus[] = ["PENDING", "APPROVED", "REJECTED"];
...
function parseCabinPayload(body: Record<string, unknown>) {
  const {
    name,
    location,
    price,
...
export async function getCabins(req: Request, res: Response) {
  try {
...
export async function getAdminCabins(_req: Request, res: Response) {
  try {
    const cabins = await prisma.cabin.findMany({
      include: {
        owner: {
...
export async function getOwnerCabins(req: Request, res: Response) {
  try {
    if (!req.user) {
...
export async function createCabin(req: Request, res: Response) {
  try {
    if (!req.user) {
      return res.status(401).json({ message: "Not authorized" });
    }
...
export async function updateCabin(req: Request<{ id: string }>, res: Response) {
  try {
    if (!req.user) {
      return res.status(401).json({ message: "Not authorized" });
    }
    ...
    const isCabinDetailsChanged =
      (payload.name !== undefined && payload.name !== existingCabin.name) ||
...
export async function updateCabinStatus(
  ...
  try {
    ...
    if (!cabinStatuses.includes(status as CabinStatus)) {
      return res.status(400).json({
...
export async function deleteCabin(req: Request<{ id: string }>, res: Response) {
  try {
    if (!req.user) {
      return res.status(401).json({ message: "Not authorized" });
    }
