...
export async function createGift(_req: Request, res: Response) {
...
export async function getReceivedGifts(req: Request, res: Response) {
  try {
    ...
    if (req.user.role !== "USER") return res.status(403).json({ message: "Only guests can receive gifts" });
...
export async function getSentGifts(req: Request, res: Response) {
  try {
    if (!req.user) return res.status(401).json({ message: "Not authorized" });
    if (req.user.role !== "USER") return res.status(403).json({ message: "Only guests can send gifts" });
...
export async function acceptGift(req: Request<{ id: string }>, res: Response) {
  try {
    if (!req.user) return res.status(401).json({ message: "Not authorized" });
    if (req.user.role !== "USER") return res.status(403).json({ message: "Only guests can accept gifts" });
...
export async function declineGift(req: Request<{ id: string }>, res: Response) {
  try {
    if (!req.user) return res.status(401).json({ message: "Not authorized" });
    if (req.user.role !== "USER") return res.status(403).json({ message: "Only guests can decline gifts" });
...
export async function getOwnerGiftRequests(req: Request, res: Response) {
  try {
    if (!req.user) return res.status(401).json({ message: "Not authorized" });
...
export async function updateOwnerGiftStatus(req: Request<{ id: string }>, res: Response) {
  try {
    if (!req.user) return res.status(401).json({ message: "Not authorized" });
