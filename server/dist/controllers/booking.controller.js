...
const paymentMethods = ["PAY_AT_PROPERTY", "GIFT_CREDIT", "ESEWA"];
...
async function withSuccessfulBookingCounts(bookings) {
    const userIds = Array.from(new Set(bookings.map((booking) => booking.user?.id).filter(Boolean)));
    if (userIds.length === 0)
        return bookings;
    ...
    const countMap = new Map(grouped.map((item) => [item.userId, item._count._all]));
...
export async function getMyBookings(req, res) {
    try {
        if (!req.user)
            return res.status(401).json({ message: "Not authorized" });
        if (req.user.role !== "USER")
            return res.status(403).json({ message: "Only customer accounts have personal bookings" });
...
export async function getOwnerBookingById(req, res) {
    try {
        if (!req.user)
            return res.status(401).json({ message: "Not authorized" });
        await markCompletedBookings();
        ...
        if (!booking)
            return res.status(404).json({ message: "Booking not found for your cabins" });
        const [bookingWithStats] = await withSuccessfulBookingCounts([booking]);
        return res.status(200).json({ message: "Booking details fetched successfully", booking: bookingWithStats });
    }
    catch (error) {
...
export async function updateMyBookingPayment(req, res) {
    try {
        if (!req.user)
            return res.status(401).json({ message: "Not authorized" });
        if (req.user.role !== "USER")
            return res.status(403).json({ message: "Only customer accounts can pay for bookings" });
        const { id } = req.params;
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            if (booking.status !== "CONFIRMED")
                throw new Error("Payment can be selected only after the owner confirms your booking");
...
export async function cancelMyBooking(req, res) {
    try {
        if (!req.user)
            return res.status(401).json({ message: "Not authorized" });
        if (req.user.role !== "USER")
            return res.status(403).json({ message: "Only customer accounts can cancel personal bookings" });
        const { id } = req.params;
